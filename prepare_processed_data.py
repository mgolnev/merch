...
def load_feed(feed_path):
    ...
    for offer in root.findall('.//offer'):
        ...
        category_ids = offer.findall('categoryId')
        category_names = set()  # Используем set для уникальности
        for cat_id in category_ids:
            if cat_id.text in categories:
                # Добавляем все категории в цепочке
                category_names.update(get_all_categories(cat_id.text))
        ...
        feed_data[sku] = {
            ...
            'categories': sorted(list(category_names))  # Сортируем для стабильности
        }
    return feed_data
...
def process_data(data_path, feed_path, output_path, dnp_path=None):
    ...
    # Добавляем категории из фида, если они есть, иначе используем max_Категория
    df_grouped['categories'] = df_grouped.apply(
        lambda row: ' | '.join(feed_data.get(row['Артикул'], {}).get('categories', []) or [row['max_Категория']]),
        axis=1
    )
    
    # Удаляем старую колонку max_Категория
    df_grouped = df_grouped.drop('max_Категория', axis=1)
    
    # Если Название товара пустое, подставляем из фида
    df_grouped['Название товара'] = df_grouped.apply(
        lambda row: row['Название товара'] if row['Название товара'] else feed_data.get(row['Артикул'], {}).get('name', ''),
        axis=1
    )
    ...
    column_rename = {
        ...
        'categories': 'categories',  # Уже переименовано выше
        'Сессии': 'sessions',
